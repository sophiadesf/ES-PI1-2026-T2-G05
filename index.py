...
import seguranca as seguranca  # Modulo de funcoes de seguranca (CPF, titulo, chave de acesso, etc.)
# =============================================================================
# MENU PRINCIPAL
        ...
        if opcaoVotacaoIniciado == 1:
            ...
            while not eleitor_valido:
                ...
                if not consultas.verifica_eleitor(titulo, cpf_4, chave, 0):
                    ...
                    seguranca.registrar_log_tentativa_acesso_negado("Validação de eleitor falhou")
                    continue
                
                if consultas.verifica_javotou(titulo, cpf_4, chave):
                    print("\nErro: Eleitor invalido! Esse eleitor já votou nessa eleição\n")
                    seguranca.registrar_log_voto_duplo()
                    continue
            ...
            if protocolo:
                ...
                seguranca.registrar_log_voto_sucesso()
            print("----------------------------------\n")
        ...
        # RF002.01.07 - Encerrar sistema de votação
            ...
            while not mesario_valido:
                ...
                if not consultas.verifica_eleitor(titulo, cpf_4, chave, 1):
                    ...
                    seguranca.registrar_log_tentativa_acesso_negado("Validação de mesário falhou")
                    continue
            ...
            if is_enc_votacao == 'N':
                ...
                while not chave_valida:
                    ...
                    if not consultas.verifica_eleitor(titulo, cpf_4, chave_acesso, 1):
                        ...
                        seguranca.registrar_log_tentativa_acesso_negado("Chave de acesso inválida para encerramento")
                        continue
                ...
                if consultas.fechar_urna():
                    ...
                    seguranca.registrar_log_encerramento()
                    break
                else:
...
while opcao != 3: 
        ...
        while opcaoGerenciamento != 0:
            ...
            if opcaoGerenciamento == 1:
                ...
                while not nome_valido:
                    ...
                    if len(partes) < 2:
                        ...
                        seguranca.salvar_log("ERRO - Tentativa de cadastro sem nome e sobrenome")
                        nome_valido = False
                        continue
                ...
                while not cpf_valido :
                    ...
                    if not seguranca.validar_cpf(cpf):
                        print("\nErro: CPF invalido! Verifique os digitos. Digite um cpf válido\n")
                        seguranca.salvar_log("ERRO - CPF invalido informado")
                        continue
                    ...
                    if consultas.verificar_cpf_existe(cpf):
                        ...
                        seguranca.salvar_log("ERRO - Tentativa de cadastro com CPF duplicado")
                        continue
                ...
                while not titulo_valido:
                    ...
                    if not seguranca.validar_titulo(titulo):
                        print("\nErro: Titulo inválido! Verifique os digitos e digite novamente!\n")
                        seguranca.salvar_log("ERRO - Titulo de eleitor invalido informado")
                        continue
                    ...
                    if consultas.verificar_titulo_existe(titulo):
                        ...
                        seguranca.salvar_log("ERRO - Tentativa de cadastro com Titulo de eleitor duplicado")
                        continue
                ...
                chave_acesso = seguranca.gerar_chave_acesso(nome)
                ...
                try:
                    ...
                    seguranca.salvar_log("SUCESSO - Eleitor cadastrado: " + nome)
                    ...
                    seguranca.salvar_log("ERRO - Falha ao cadastrar eleitor: " + str(e))
            ...
            # ---------------------------------------------------------
                ...
                if not eleitor_encontrado:
                    ...
                    seguranca.salvar_log("ERRO - Eleitor nao encontrado para edicao: " + cpf_busca)
                    continue
                ...
                if opcao_edicao == 0:
                    ...
                    try:
                        ...
                        seguranca.salvar_log("SUCESSO - Eleitor editado: " + cpf_busca)
                        
                    except Exception as e:
                        print("\nErro ao atualizar: " + str(e) + "\n")
                        seguranca.salvar_log("ERRO - Falha ao editar eleitor: " + str(e))
                else:
                    print("\nOpcao invalida!\n")
            ...
            # ---------------------------------------------------------
                ...
                try:
                    ...
                    print ("\nErro ao buscar: " + str(e) + "\n")
                    seguranca.salvar_log("ERRO - Falha ao buscar eleitor para remocao: " + str(e))
                    continue
                
                # Verificar se eleitor foi encontrado
                if not eleitor_encontrado:
                    print("\nEleitor nao encontrado!\n")
                    seguranca.salvar_log("ERRO - Eleitor nao encontrado para remocao: " + cpf_busca)
                    continue
                ...
                if confirmacao != 'S':
                    print("\nRemocao cancelada.\n")
                    seguranca.salvar_log("GERENCIAMENTO - Remocao de eleitor cancelada: " + cpf_busca)
                    continue
                ...
                try:
                    ...
                    seguranca.salvar_log("SUCESSO - Eleitor removido: " + cpf_busca + " (" + eleitor_encontrado[1] + ")")
                    
                except Exception as e:
                    print("\nErro ao remover: " + str(e) + "\n")
                    seguranca.salvar_log("ERRO - Falha ao remover eleitor: " + str(e))
            ...
            # ---------------------------------------------------------
                ...
                try:
                    ...
                    seguranca.salvar_log("ERRO - Falha ao buscar eleitores: " + str(e))
            ...
            # ---------------------------------------------------------
                ...
                while opcaoCandidatos != 0:
                    ...
                    if opcaoCandidatos == 1:
                        ...
                        seguranca.salvar_log("GERENCIAMENTO CANDIDATOS - Cadastrar")
                        ...
                        while not numero_valido:
                            try:
                                ...
                                if consultas.verificar_numero_candidato_existe(numero):
                                    ...
                                    seguranca.salvar_log("ERRO - Tentativa de cadastro com número duplicado")
                                    continue
                        ...
                        try:
                            ...
                            if id_candidato:
                                ...
                                print("="*40 + "\n")
                                seguranca.salvar_log(f"SUCESSO - Candidato cadastrado: {nome} (Nº {numero})")
                            else:
                                print("\nErro ao cadastrar candidato\n")
                        except Exception as e:
                            print(f"\nErro ao cadastrar: {str(e)}\n")
                            seguranca.salvar_log(f"ERRO - Falha ao cadastrar candidato: {str(e)}")
                    ...
                    # -------------------------------------------------
                        ...
                        seguranca.salvar_log("GERENCIAMENTO CANDIDATOS - Editar")
                        ...
                        try:
                            ...
                            if not candidato:
                                ...
                                seguranca.salvar_log(f"ERRO - Candidato não encontrado: {numero_busca}")
                                continue
                            ...
                            if opcao_edicao == 0:
                                ...
                                if consultas.atualizar_candidato(numero, numero_novo, nome, partido):
                                    ...
                                    print("="*40 + "\n")
                                    seguranca.salvar_log(f"SUCESSO - Candidato editado: número {numero} → {numero_novo}")
                                else:
                                ...
                                if consultas.atualizar_candidato(numero, numero, nome_novo, partido):
                                    ...
                                    print("="*40 + "\n")
                                    seguranca.salvar_log(f"SUCESSO - Candidato editado: {nome} → {nome_novo}")
                                else:
                                    print("\nErro ao atualizar candidato\n")
                                ...
                                if consultas.atualizar_candidato(numero, numero, nome, partido_novo):
                                    ...
                                    print("="*40 + "\n")
                                    seguranca.salvar_log(f"SUCESSO - Candidato editado: partido {partido} → {partido_novo}")
                                else:
                                    print("\nErro ao atualizar candidato\n")
                            else:
                                print("\nOpção inválida!\n")
                            ...
                            print(f"\nErro: {str(e)}\n")
                            seguranca.salvar_log(f"ERRO - Falha ao editar candidato: {str(e)}")
                    ...
                    # -------------------------------------------------
                        ...
                        seguranca.salvar_log("GERENCIAMENTO CANDIDATOS - Remover")
                        ...
                        try:
                            ...
                            if not candidato:
                                print("\nCandidato não encontrado!\n")
                                seguranca.salvar_log(f"ERRO - Candidato não encontrado para remoção: {numero_busca}")
                                continue
                            ...
                            if confirmacao != 'S':
                                print("\nRemoção cancelada.\n")
                                seguranca.salvar_log(f"GERENCIAMENTO CANDIDATOS - Remoção cancelada: {numero}")
                                continue
                            ...
                            if consultas.remover_candidato(numero):
                                ...
                                print("Nome: " + nome)
                                print("="*40 + "\n")
                                seguranca.salvar_log(f"SUCESSO - Candidato removido: {nome} (Nº {numero})")
                            else:
                            ...
                            print(f"\nErro: {str(e)}\n")
                            seguranca.salvar_log(f"ERRO - Falha ao remover candidato: {str(e)}")
                    ...
                    # -------------------------------------------------
                        ...
                        seguranca.salvar_log("GERENCIAMENTO CANDIDATOS - Buscar")
                    ...
                    # -------------------------------------------------
                        ...
                        seguranca.salvar_log("GERENCIAMENTO CANDIDATOS - Listar")
                        ...
                        seguranca.salvar_log("GERENCIAMENTO CANDIDATOS - ERRO: Opção inválida")
            ...
            # Opcao invalida
                ...
                seguranca.salvar_log("Gerenciamento -> ERRO: Opcao invalida do menu")
    ...
    # =================================================================
        ...
        seguranca.salvar_log("Opcao de votacao selecionado")

        if consultas.urna_aberta():
            print("\nUrna ja esta aberta!\n")
            menu_votacao_iniciada()

            seguranca.salvar_log("VOTACAO - Urna ja aberta")
        else:
            opcaoVotacao = -1
            while opcaoVotacao != 0:
                ...
                if opcaoVotacao == 1:
                    ...
                    seguranca.registrar_log_abertura()
                    menu_votacao_iniciada()
                    print("----------------------------------\n")
                    ...
                    while opcaoResultados != 0:
                        ...
                        if opcaoResultados == 1: 
                            ...
                            seguranca.salvar_log("VOTACAO - Resultado - ERRO: Selecionado opção inválida do menu")
                            print("Opção inválida")
                ...
                # ---------------------------------------------------------
                    ...
                    while opcaoVotacaoAuditoria != 0:
                        ...
                        if opcaoVotacaoAuditoria == 1:
                            ...
                            seguranca.exibir_logs()
                            seguranca.salvar_log("AUDITORIA - Exibição de Logs de Ocorrências")
                     ...
                     # RF002.02.02 - Exibição dos Protocolos de Votação de votacao
                            ...
                            seguranca.salvar_log("AUDITORIA - Exibição dos Protocolos de Votação")
                            ...
                            seguranca.salvar_log("Votacao -> Auditoria -> ERRO: Opcao invalida do menuß")

                elif opcaoVotacao == 0:
                    print("Saindo do modulo de votacao...\n")
                    seguranca.salvar_log("VOTACAO - Sair")
                
                else:
                    print("Votacao -> Opcao invalida\n")
                    seguranca.salvar_log("Votacao -> Opcao invalida")
